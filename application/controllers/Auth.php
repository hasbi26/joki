...
class Auth extends CI_Controller
{
	...
	private function _sendEmail($token, $type)
	{
		...
		$confing = [
			...
			'smtp_user' => '3nginexxx@gmail.com',
			'smtp_pass' => 'vnci exnh eijk uxrl',
			'smtp_port' => 465,
			'mailtype'  => 'html',
			'charset'   => 'utf-8',
			'newline'   => "\r\n"
		...
		$this->email->from('3nginexxx@gmail.com', 'Joki Universe');
		$this->email->to($email);
	...
	public function logout()
	{
		...
		$cache_deleted = $this->delete_cache($uri_string);
		var_dump("strn", $cache_deleted);
		$this->session->unset_userdata('email');
		$this->session->unset_userdata('role_id');
		$this->Flasher_model->flashdata('You have been logout'.$cache_deleted, 'Success'.$cache_deleted, 'success'.$cache_deleted);
		redirect('auth');
	}
